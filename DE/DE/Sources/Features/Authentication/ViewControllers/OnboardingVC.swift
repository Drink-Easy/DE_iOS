...
public class OnboardingVC: UIViewController, UICollectionViewDelegate {
    ...
    lazy var pageControl = CustomPageControl(indicatorColor: .white, currentIndicatorColor: .white).then {
    ...
    private func setupUI() {
        ...
        pageControl.snp.makeConstraints { make in
            ...
            make.bottom.equalTo(startButton.snp.top).inset(-Constants.padding32)
            make.width.equalTo(100) // 페이지 컨트롤 너비 설정
        }
        startButton.snp.makeConstraints { make in
            make.leading.trailing.equalTo(view.safeAreaLayoutGuide).inset(Constants.padding32)
            make.bottom.equalTo(view.safeAreaLayoutGuide).inset(Constants.padding32 * 2)
