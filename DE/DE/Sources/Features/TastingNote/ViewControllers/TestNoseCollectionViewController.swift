...
extension TestNoseCollectionViewController: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    public func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        let header = collectionView.dequeueReusableSupplementaryView(ofKind: kind, withReuseIdentifier: "Header", for: indexPath) as! NoseHeaderView
        header.configure(with: sections[indexPath.section].sectionTitle, section: indexPath.section, delegate: self)
        return header
    }
}
...
class NoseHeaderView: UICollectionReusableView {
    ...
    // MARK: - 데이터 설정
    func configure(with title: String, section: Int, delegate: NoseHeaderViewDelegate) {
        titleLabel.text = title // 섹션 제목 설정
        self.section = section // 섹션 번호 저장
        self.delegate = delegate // 델리게이트 저장
    }
