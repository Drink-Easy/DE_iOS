...
class WineInfoView: UIView {
    ...
    private let fixLabel: UILabel = {
        let f = UILabel()
        f.text = "수정하기"
        f.font = UIFont.ptdRegularFont(ofSize: 12)
        f.textColor = UIColor(hex: "#767676")
        return f
    }()
    ...
    private let polygonView: PolygonChartView = {
        let v = PolygonChartView()
    ...
    func setupUI() {
        ...
        graphView.addSubview(fixLabel)
        fixLabel.snp.makeConstraints { make in
            make.trailing.equalTo(graphVector.snp.trailing)
            make.bottom.equalTo(graphVector.snp.top).offset(-6)
        }
        ...
        polygonView.snp.makeConstraints { make in
            ...
            make.bottom.equalTo(graphVector.snp.bottom).offset(319)
