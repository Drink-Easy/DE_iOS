...
public final class RecommentService: NetworkManager {
    typealias Endpoint = RecommentEndpoints
    
    // Provider 설정
    let provider: MoyaProvider<RecommentEndpoints>
    
    public init(provider: MoyaProvider<RecommentEndpoints>? = nil) {
        // 플러그인 추가
    ...
    /// 대댓글 등록 데이터 생성
    public func makePostDTO(commentId: Int, content: String) -> RecommentRequestDTO {
    ...
    /// 대댓글 작성 API
    public func postComment(data: RecommentRequestDTO, completion: @escaping (Result<String, NetworkError>) -> Void) {
        request(target: .postRecomments(requestDTO: data), decodingType: String.self, completion: completion)
    }
    
    /// 대댓글 삭제 API
    public func deleteComment(recommentId: Int, completion: @escaping (Result<String, NetworkError>) -> Void) {
