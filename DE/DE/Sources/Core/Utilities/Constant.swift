...
10. '콘텐츠'란 정보통신망법의 규정에 따라 정보통신망에서 사용되는 부호 ·문자·음성·음향·이미지 또는 영상 등으로 정보 형태의 글, 사진, 동영상 및 각종 파일과 링크 등을 말합니다.
...
3. 회사가 제공하는 서비스의 내용은 다음과 같습니다.
    1. 헌혈 커뮤니티, 헌혈 크루, 지정헌혈 홍보에 대한 서비스를 제공합니다.
    2. 회원은 헌혈증서를 등록하여 자신의 헌혈 횟수를 확인할 수 있
...
2. 전항의 규정에도 불구하고 아래 각 호의 경우에는 결제한 전액을 환불합니다. 결제를 완료한 후 서비스 이용 내역이 없는 경우 서비스 장애 등 회사의 귀책사유로 서비스를 이용하지 못한 경우 구매한 서비스가 제공되지 않은 경우 구매한 서비스가 표시 또는 광고 등과 상이하거나 현저한 차이가 있는 경우 서비스 자체의 결함으로 서비스 이용이 현저히 불가능 하였던 경우
